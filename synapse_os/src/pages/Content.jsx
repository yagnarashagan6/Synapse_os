...
const PlatformIcon = ({ platform }) => {
  switch (platform?.toLowerCase()) {
    ...
    case "tiktok":
      return <Music2 size={16} className="text-white" />;
    default:
...
const Content = () => {
    ...
    return [
      {
        ...
        description: "Carousel post showcasing new AI design tools.",
        platform: "instagram",
        status: "new",
        date: "Oct 24",
      },
      {
        id: 2,
        title: "Future of Education Trends",
        description: "Deep dive video script regarding EduGen analysis.",
        platform: "youtube",
        status: "in-progress",
        date: "Oct 25",
      },
      {
        ...
        description: "Thread exploring efficient remote work setups.",
        platform: "twitter",
        status: "in-progress",
        date: "Oct 26",
      },
      {
        ...
        description: "Short-form video for eco-friendly gadgets.",
        platform: "tiktok",
        status: "approved",
        date: "Oct 28",
      },
      {
        id: 5,
        title: "Synapse OS Feature Launch",
        description: "Official press release and professional update.",
        platform: "linkedin",
        status: "approved",
        date: "Oct 30",
      },
  ...
  const fetchVideos = (forceSync = false) => {
    ...
    getVideos()
      .then((data) => {
        ...
        if (!globalVideosCache) setLoadingVideos(false);
      })
      .catch((err) => {
  ...
  const handleBulkMove = async (itemsToMove, targetStatus) => {
    if (targetStatus === "delete") {
      if (
        !window.confirm(
          `Are you sure you want to delete ${itemsToMove.length} items?`,
        )
      )
        return;
      const tasksToDelete = itemsToMove
        .filter((i) => i.type === "task")
        .map((i) => i.id);
      if (tasksToDelete.length > 0) {
        setTasks((prev) => prev.filter((t) => !tasksToDelete.includes(t.id)));
      }
      const videosToDelete = itemsToMove
      ...
      const tasksToUpdate = itemsToMove
        .filter((i) => i.type === "task")
        .map((i) => i.id);
      if (tasksToUpdate.length > 0) {
        setTasks((prev) =>
          prev.map((t) =>
            tasksToUpdate.includes(t.id) ? { ...t, status: targetStatus } : t,
          ),
        );
      }
      const videosToUpdate = itemsToMove
        .filter((i) => i.type === "video")
        .map((i) => i.id);
    ...
    >
      <div className="flex items-center justify-between mb-3">
        ...
        <Button
          variant="ghost"
          size="icon"
          className="h-6 w-6 opacity-0 group-hover:opacity-100 p-0 text-slate-500"
        >
          <MoreHorizontal size={14} />
        </Button>
      </div>

      <h4 className="font-medium text-slate-200 mb-2 line-clamp-2">
        {item.title}
      </h4>
      ...
      {/* Action / Next step */}
      <div
        className="flex items-center justify-end pt-3 mt-3 border-t border-slate-700/50"
        onClick={(e) => e.stopPropagation()}
      >
  ...
  const VideoCard = ({ video }) => {
    ...
    const isGif =
      ...
      video.video_url?.toLowerCase().includes(".webp") ||
      video.video_url?.toLowerCase().includes("/gif/");
    ...
    return (
      <div className="w-[320px] bg-slate-800/80 backdrop-blur-md border border-purple-500/30 rounded-2xl overflow-hidden group hover:border-purple-500 hover:shadow-[0_0_20px_rgba(168,85,247,0.2)] transition-all flex-shrink-0 relative">
        ...
        <div className="p-4 relative">
          ...
          <h4 className="font-bold text-slate-200 text-sm line-clamp-1 mb-4">
            {video.topic || "Untitled Creation"}
          </h4>
  ...
  const getColumnItems = (status) => {
    ...
    const filteredVideos = videos
      ...
      .map((v) => ({
        ...
        description:
          v.status_message ||
          "Generated video content mapped to workflow queue.",
  ...
  const UnifiedColumn = ({ title, status, color }) => {
    ...
    const selectedInColumn = items.filter((item) =>
    ...
    return (
      ...
      >
        <div
          className={`p-4 border-b border-slate-800/50 flex items-center justify-between sticky top-0 bg-slate-900/90 backdrop-blur-md z-10 rounded-t-xl border-l-4 ${color}`}
        >
        ...
        <div className="p-3 border-t border-slate-800/50">
          {status === "approved" ? (
            ...
            >
              &#8644; Move{" "}
              {hasSelection ? `${selectedInColumn.length} Selected` : "All"} to
              Approvals
            </button>
          ) : (
            <select
            ...
            >
              <option value="" disabled>
                &#8644; Move{" "}
                {hasSelection ? `${selectedInColumn.length} Selected` : "All"}{" "}
                To...
              </option>
              <option value="new" disabled={status === "new"}>
                New Ideas
              </option>
              <option value="in-progress" disabled={status === "in-progress"}>
                In Progress
              </option>
              <option value="approved" disabled={status === "approved"}>
                Approved
              </option>
              <option value="rejected" disabled={status === "rejected"}>
                Rejected
              </option>
              {status === "rejected" && (
  ...
  const uncategorizedVideos = videos.filter(
    (v) =>
      !["new", "in-progress", "approved", "rejected", "in-approvals"].includes(
        v.status,
      ),
  );
        ...
        <div className="mb-10">
          <div className="flex items-center gap-3 mb-6">
            ...
            <h3 className="text-xl font-bold text-white">
              Created Videos ({uncategorizedVideos.length})
            </h3>
          </div>
